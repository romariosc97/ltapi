...
//module.exports = (io) => {
      ...
      credentials.store(req)
        .then(sf => {
          ...
          my_io.on('connection', socket => {
            socket.join(sf.auth_response.id);
            console.log('Room created with ID: '+sf.auth_response.id);
  ...
  const destroyConnection = (req, res) => {
    ...
    conn.logout((err) => {
      if (!err) {
        let room = req.session.salesforce.auth_response.id;
        req.session.destroy(() => {
          
          my_io.on('connection', socket => {
            socket.leave(room);
            console.log('Room destroyed.xx');
            console.log(socket.rooms);
          });
